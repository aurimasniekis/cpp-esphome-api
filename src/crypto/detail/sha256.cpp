...
//
// Source: Brad Conte's "crypto-algorithms" (sha256.c),
//         https://github.com/B-Con/crypto-algorithms, commit cfbde48.
//         Released into the public domain by its author.
//
// Adapted to C++: fixed-width <cstdint> types, wrapped in the project namespace,
// one-shot helper added. Algorithm unchanged. Compiled with relaxed warnings.
// ---------------------------------------------------------------------------
...
inline std::uint32_t rotr(const std::uint32_t x, const std::uint32_t n) {
    return (x >> n) | (x << (32 - n));
}
...
void sha256_update(Sha256Ctx& ctx, const std::uint8_t* data, const std::size_t len) {
...
void sha256_final(Sha256Ctx& ctx, std::uint8_t* hash) {
    ...
    // SHA uses big endian byte ordering; extract the eight state words.
    for (i = 0; i < 4; ++i) {
...
void sha256(const std::uint8_t* data, const std::size_t len, std::uint8_t* hash) {
    Sha256Ctx ctx;
    sha256_init(ctx);
